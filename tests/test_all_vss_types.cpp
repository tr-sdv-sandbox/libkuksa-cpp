...
TEST(VSSTypeCompilation, AllScalarSensorTypesWithSubscriber) {
    ...
    if (subscriber) {  // Will never execute, just for compilation
        // Bool
        {
            ...
            subscriber->subscribe(sensor, [](std::optional<bool>){});
        }

        // int32_t
        {
            auto sensor = TestResolver::signal<int32_t>("test");
            subscriber->subscribe(sensor, [](std::optional<int32_t>){});
        }

        // uint32_t
        {
            auto sensor = TestResolver::signal<uint32_t>("test");
            subscriber->subscribe(sensor, [](std::optional<uint32_t>){});
        }

        // int64_t
        {
            auto sensor = TestResolver::signal<int64_t>("test");
            subscriber->subscribe(sensor, [](std::optional<int64_t>){});
        }

        // uint64_t
        {
            auto sensor = TestResolver::signal<uint64_t>("test");
            subscriber->subscribe(sensor, [](std::optional<uint64_t>){});
        }

        // float
        {
            auto sensor = TestResolver::signal<float>("test");
            subscriber->subscribe(sensor, [](std::optional<float>){});
        }

        // double
        {
            auto sensor = TestResolver::signal<double>("test");
            subscriber->subscribe(sensor, [](std::optional<double>){});
        }

        // string
        {
            auto sensor = TestResolver::signal<std::string>("test");
            subscriber->subscribe(sensor, [](std::optional<std::string>){});
        }
    }
}
